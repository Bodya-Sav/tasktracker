...
const setSlideStyles = (
  ...
  if (inView) {
    ...
    if (index === selectedIndex) {
      ...
      slideNode.style.color = "white";
    } else {
      slideNode.style.opacity = "0.3";
      slideNode.style.color = "gray";
    }
  } else {
...
export const IosPickerItem: React.FC<IosPickerItemProps> = (props) => {
  ...
  useEffect(() => {
    ...
    if (emblaApi && !hasScrolled.current && initialValue) {
    ...
    inactivateEmblaTransform(emblaApi);
    rotateWheel(emblaApi);
  }, [emblaApi, inactivateEmblaTransform, rotateWheel, onSelect, initialValue]);
          ...
          <div className="embla__ios-picker__container">
            {slides.map((index) => (
              <div className="embla__ios-picker__slide" key={index}>
                {formatValue ? formatValue(index) : index}
              </div>
            ))}
          </div>
        </div>
...
export const EmblaCarousel: React.FC<EmblaCarouselProps> = (props) => {
  const { loop, onHourSelect, onMinuteSelect, initialHour, initialMinute } =
    props;
  ...
  return (
    <div className="embla">
      <IosPickerItem
        ...
        formatValue={(n) => n.toString().padStart(2, "0")}
      />
      <IosPickerItem
        slideCount={60}
        perspective="right"
